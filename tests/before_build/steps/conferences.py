...
def step_impl(context):
    for conf in context.conferences:
        ...
        assert url_regex.match(conf.get('google_maps_pin')), "'google_maps_pin' is not a valid URL"

        assert isinstance(datetime.datetime.strptime(conf.get('start_time'), '%Y-%m-%d'),
                          datetime.datetime), "'start_time' needs to be in the datetime format"
        assert isinstance(datetime.datetime.strptime(conf.get('end_time'), '%Y-%m-%d'),
                          datetime.datetime), "'end_time' needs to be in the datetime format"

        assert datetime.datetime.strptime(conf.get('start_time'), '%Y-%m-%d') <= datetime.datetime.strptime(
            conf.get('end_time'), '%Y-%m-%d'), "'start_time' needs to before 'end_time'"

        assert len(conf.get('blurb')) < 300, "'blurb' value is more than 300 characters"
        if conf.get('url'):
            assert url_regex.match(conf.get('url')), "'url' is not a valid URL"
        if conf.get('funnel'):
            assert url_regex.match(conf.get('funnel')), "'funnel' is not a valid URL"

        if conf.get('color'):
            assert color_regex.match(conf.get('color').get('primary')), "'primary' color is not a valid color"
            assert color_regex.match(conf.get('color').get('primary_dark')), "'primary_dark' color is not a valid color"
            assert color_regex.match(conf.get('color').get('accent')), "'accent' color is not a valid color"
